...
Write-Host "✓ Paquete distribuible creado" -ForegroundColor Green
Write-Host ""
...
if ($exeExists -and $assetsExist) {
    ...
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Ubicación: .\MineCEepper-dist\MineCEepper.exe" -ForegroundColor White
    Write-Host ""
    ...
    if (-not $exeExists) { Write-Host "  - No se encontró el ejecutable" -ForegroundColor Red }
    if (-not $assetsExist) { Write-Host "  - No se encontraron los assets" -ForegroundColor Red }
    exit 1
}
